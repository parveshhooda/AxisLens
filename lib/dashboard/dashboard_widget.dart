  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      ...
      if ((_model.apiResultj1o?.succeeded ?? true)) {
        await queryRewardsLogicRecordOnce()
            .then(
              (records) => _model.simpleSearchResults = TextSearch(
                records
                    .map(
                      (record) => TextSearchItem(record, [record.priority!]),
                    )
                    .toList(),
              ).search('P').map((r) => r.object).toList(),
            )
            .onError((_, __) => _model.simpleSearchResults = [])
            .whenComplete(() => setState(() {}));

        if (GetAcceptedIssuesCall.total(
              (_model.apiResultj1o?.jsonBody ?? ''),
            ) ==
            0) {
          setState(() {
            FFAppState().LP = 0;
          ...
          setState(() {
            FFAppState().RewardsValue =
                _model.simpleSearchResults.first.rewardValue;
            FFAppState().AcceptedAPIPriorities =
                (GetAcceptedIssuesCall.priorityList(
              (_model.apiResultj1o?.jsonBody ?? ''),
            ) as List)
          ...
          _model.lp = await actions.findTotalLP();
          setState(() {
            FFAppState().LP = valueOrDefault<int>(
              _model.lp,
              0,
            );
          });
        ...
        setState(() {
          FFAppState().APIFialureFlag = false;
        });
        setState(() {
          FFAppState().isLoading = false;
        });

        context.goNamed('Failure');
      }
    });
                                                ...
                                                child: ClipRRect(
                                                  borderRadius:
                                                      BorderRadius.circular(
                                                          8.0),
                                                  child: Image.asset(
                                                    'assets/images/AL_WeeklyChallenge.jpg',
                                                    width:
                                                      ...
                                                      padding:
                                                          EdgeInsetsDirectional
                                                              .fromSTEB(
                                                                  ...
                                                                  2.0),
                                                      child: Text(
                                                        '1. Place for users to raise a issue faced during testing Digital channels like Axis Mobile  App, Internet Banking',
                                                        textAlign:
                                                            TextAlign.start,
                                                        style:
                                                            FlutterFlowTheme.of(
                                                      ...
                                                      padding:
                                                          EdgeInsetsDirectional
                                                              .fromSTEB(
                                                                  ...
                                                                  2.0),
                                                      child: Text(
                                                        '3. LPs can be redeemed as Voucher',
                                                        textAlign:
                                                            TextAlign.start,
                                                        style:
                                                            FlutterFlowTheme.of(
